...
def create_question(): #function to create new question in database
    try:
        ...
        data=list(db.Questions.find({"_id": ObjectId(dbResponse.inserted_id)}))
...
def update_question(id): #function to update the data of question with specific id in database
    try:
        dbResponse = db.Questions.update_one({"_id": ObjectId(id)},{"$set": {"qtext": request.form["qtext"],"qtype": request.form["qtype"],"domain": request.form["domain"]}})
...
def delete_question(id): #function to delete question from database 
    try:
        data = list(db.Questions.find({"_id": ObjectId(id)}))
        for question_id in data:
            question_id["_id"]=str(question_id["_id"])

        dbResponse = db.Questions.delete_one({"_id": ObjectId(id)})
